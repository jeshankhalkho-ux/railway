...
function httpsGet(path: string, cookies: string): Promise<{ body: string; cookies: string }> {
  return new Promise((resolve, reject) => {
    const opts: https.RequestOptions = {
      hostname: BASE_HOST, path: BASE_PATH + path, port: 443,
...
function httpsPost(path: string, body: Record<string, string>, cookies: string): Promise<{ body: string; cookies: string }> {
  return new Promise((resolve, reject) => {
...
async function fetchNTES(endpoint: string, form: Record<string, string>): Promise<string> {
  ...
  const r = await httpsPost(endpoint + (endpoint.includes('?') ? '&' : '?') + 'lan=en', { ...form, [m[1]]: m[2] }, tokenBody.cookies);
  return r.body;
}
...
function getDatePane($: cheerio.CheerioAPI, date: string) {
  ...
  if (!pane || !pane.find('.stopRow').length) {
    $('.tab-pane').each((_i: number, p: any) => { const $p = $(p); if ($p.find('.stopRow').length > 0 && !pane) pane = $p; });
  }
  return pane;
}

function parseStationRow($: cheerio.CheerioAPI, $row: any) {
  const nc = $row.children('div').filter((_i: number, el: any) => ($(el).attr('style')||'').includes('float:right'))
    .children('div').filter((_i: number, el: any) => ($(el).attr('style')||'').includes('float:left'));
  ...
  const rf = $row.children('div').filter((_i: number, el: any) => ($(el).attr('style')||'').includes('float:right')&&($(el).attr('style')||'').includes('flex'));
  const rt = rf.text();
  let type = 'STOP';
  ...
  const lt = $row.children('div').filter((_i: number, el: any) => ($(el).attr('style')||'').includes('float:left')&&($(el).attr('style')||'').includes('width:100px')).first();
  ...
  const rtime = rf.children('div').filter((_i: number, el: any) => ($(el).attr('style')||'').includes('float:right')&&($(el).attr('style')||'').includes('text-align:right')).first();
...
function parseSpotTrain(html: string, trainNo: string, date: string) {
  ...
  $('.w3-panel.w3-round.w3-blue h3').each((_i: any, el: any) => { const t = $(el).text().trim(); if (/\d{5}/.test(t)) trainName = t; });
  const ap = getDatePane($, date);
  const rows = ap ? ap.find('.stopRow') : $('.stopRow');
  rows.each((_i: any, r: any) => stations.push(parseStationRow($, $(r))));
...
function parseTrainSchedule(html: string, trainNo: string) {
  ...
  $('table.table-bordered').last().find('tbody tr').each((_i: any, row: any) => {
...
function parseLiveStation(html: string, station: string, date: string) {
  ...
  $('tr').filter((_i: number, r: any): boolean => /^\d+$/.test($(r).find('>td').first().text().trim())).each((_i: any, row: any) => {
    const tds = $(row).find('>td');
...
function parseTrainsBetween(html: string, from: string, to: string, date: string) {
  ...
  $('tr.w3-round').each((_i: any, row: any) => {
    const r = $(row);
